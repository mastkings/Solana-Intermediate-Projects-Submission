...
pub mod staking_program {
    ...
    pub fn airdrop(ctx: Context<Airdrop>, amount: u64) -> Result<()> {
        // Generate mint authority seeds and signer
    ...
    pub fn unstake(ctx: Context<Staking>, amount: u64) -> Result<()> {
        ...
        // Generate staking authority seeds and signer
...
impl<'info> Airdrop<'info> {
    /// Creates a CPI context for the `MintTo` instruction, enabling token minting.
    /// This function configures the CPI context required to execute the `MintTo` instruction
    /// for transferring tokens from the mint to the user's token account.
    ///
    /// Returns:
    /// - A configured CPI context for the `MintTo` instruction.
    pub fn mint_to_ctx(&self) -> CpiContext<'_, '_, '_, 'info, MintTo<'info>> {
        // Get the target program to execute the CPI on
        let cpi_program = self.token_program.to_account_info();

        // Define the accounts involved in the `MintTo` instruction
        let cpi_accounts = MintTo {
            // The mint account from which tokens are being minted
            mint: self.token_mint.to_account_info(),
            // The target user's token account to receive the minted tokens
            to: self.user_token_account.to_account_info(),
            // The authority controlling the minting process
...
impl<'info> Staking<'info> {
    // Creates a CPI context for the `Transfer` instruction to move tokens from user to vault.
    // This function configures the CPI context required to execute the `Transfer` instruction
    // for moving tokens from the user's token account to the staking vault.
    //
    // Returns:
    // - A configured CPI context for the `Transfer` instruction.
    pub fn transfer_to_vault_ctx(&self) -> CpiContext<'_, '_, '_, 'info, Transfer<'info>> {
        // Get the target program to execute the CPI on
        let cpi_program = self.token_program.to_account_info();

        // Define the accounts involved in the `Transfer` instruction
        let cpi_accounts = Transfer {
            // Source token account from which tokens are transferred (user's account)
            from: self.user_token_account.to_account_info(),
            // Destination token account where tokens are deposited (staking vault)
            to: self.staking_token_account.to_account_info(),
            // The authority controlling the token transfer (user)
    ...
    // Creates a CPI context for the `Transfer` instruction to move tokens from vault to user.
    // This function configures the CPI context required to execute the `Transfer` instruction
    // for moving tokens from the staking vault back to the user's token account.
    //
    // Returns:
    // - A configured CPI context for the `Transfer` instruction.
    pub fn transfer_to_user_ctx(&self) -> CpiContext<'_, '_, '_, 'info, Transfer<'info>> {
        ...
        // Define the accounts involved in the `Transfer` instruction
        let cpi_accounts = Transfer {
            // Source token account from which tokens are transferred (staking vault)
            from: self.staking_token_account.to_account_info(),
            // Destination token account where tokens are deposited (user's account)
            to: self.user_token_account.to_account_info(),
            // The authority controlling the token transfer (staking authority)
